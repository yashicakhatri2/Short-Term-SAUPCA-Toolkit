...
% SDS Propagation Method for state and STT.
%
% ========================================================================
% SUBFUNCTIONS GENERATED USING MATLAB SYMBOLIC TOOLBOX:
% getJOrder2: First order coded jacobian for Linear Dynamics Tensor (LDT), created using script SymbolicSDSJ2Computer
% getJOrder2: Second order coded jacobian for Linear Dynamics Tensor (LDT), created using script SymbolicSDSJ2Computer
...
% dS: State EOMs
% phiDot: First order STTdot in matrix form
% phiDotVec: STTdot in the vector form to allow propagation
...
% 
% OUTPUT =
% dSSTTpropagation: Equations of motion vector for the ode function, S = S + dS * t
% =======================================================================
...
AMat1 = getJOrder1(G,H,J2,L,RE,b,cs,g,h,hs,ks,mu,sSun);
STM1 = reshape(S(dim+1:(dim+dim^2)),dim,dim)';
phiDot = AMat1 * STM1;
phiDotVec = reshape(phiDot',dim^2,1);

if constants.STTOrder == 2
    AMat2 = getJOrder2(G,H,J2,L,RE,b,cs,g,h,hs,ks,mu,sSun);
    phiDotVec = [phiDotVec; Compute2ndOrderSTTdot(S((dim+dim^2+1):end), AMat2, dim, AMat1, STM1)];
end
